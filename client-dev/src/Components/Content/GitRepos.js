    ...
    return (
      <ul className='w-full bg-primary'>
        {this.state.isLoading === true ? (
          ...
          <div className='flex justify-center px-3 sm:px-0 flex-col items-center justify-center text-primary '>
            <p className='font-bold text-xl mb-3'>Projects</p>
            {this.state.data.map((item) => {
              return (
                <li
                  className='bg-secondary rounded-lg p-12 m-3 text-center sm:text-justify w-64 sm:max-w-2xl sm:min-w-2xl'
                  key={item.id}
                >
                  <a
                    href={item.html_url}
